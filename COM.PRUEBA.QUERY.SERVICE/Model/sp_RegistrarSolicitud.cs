...
namespace COM.PRUEBA.QUERY.SERVICE.Model
{
    ...
    {
        internal async Task<bool> RegistrarSolicitud(string descripcion, DateTime fechaSolicitud, int tipoCompra, int estado, string direccionSolicitante, string nombre, string usuario)
        {
            ...
            var command = $"[sp_RegistrarSolicitud] {string.Join(", ", parameters.Select(p => p.ParameterName))}";
